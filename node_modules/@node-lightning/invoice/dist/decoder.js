...
const hrp_pico_1 = require("./hrp-pico");
...
function decode(invoice) {
    ...
    const { prefix, words } = bech32_1.default.decode(invoice, Number.MAX_SAFE_INTEGER);
    ...
    const wordcursor = new word_cursor_1.WordCursor(words);
    ...
    while (wordcursor.wordsRemaining > 104) {
        ...
        switch (type) {
            ...
            case field_type_1.FIELD_TYPE.PAYMENT_HASH: // p - 256-bit sha256 payment_hash
                ...
                break;
            case field_type_1.FIELD_TYPE.ROUTE: // r - variable, one or more entries containing extra routing info
                {
                    value = [];
                    const bytes = wordcursor.readBytes(len);
                    const bytecursor = new bufio_1.BufferReader(bytes);
                ...
                break;
            case field_type_1.FIELD_TYPE.EXPIRY: // x - expiry time in seconds
                value = wordcursor.readUIntBE(len);
                break;
            case field_type_1.FIELD_TYPE.FALLBACK_ADDRESS: // f - variable depending on version
                {
                    ...
                    if (version !== address_version_1.ADDRESS_VERSION.SEGWIT &&
                ...
                break;
            case field_type_1.FIELD_TYPE.SHORT_DESC: // d - short description of purpose of payment utf-8
                value = wordcursor.readBytes(len).toString("utf8");
                break;
            case field_type_1.FIELD_TYPE.PAYEE_NODE: // n - 33-byte public key of the payee node
                value = wordcursor.readBytes(len);
                if (len !== 53) {
                    unknownFields.push({ type, value });
                    continue;
                ...
                break;
            case field_type_1.FIELD_TYPE.HASH_DESC: // h - 256-bit sha256 description of purpose of payment
                value = wordcursor.readBytes(len);
                if (len !== 52) {
                    unknownFields.push({ type, value });
                    continue;
                ...
                break;
            case field_type_1.FIELD_TYPE.MIN_FINAL_CLTV_EXPIRY: // c - min_final_cltv_expiry to use for the last HTLC in the route
                value = wordcursor.readUIntBE(len);
                break;
            default:
                value = wordcursor.readBytes(len);
    ...
    const payeeNodeField = fields.find(p => p.type === field_type_1.FIELD_TYPE.PAYEE_NODE);
    ...
    if (!crypto.ecdsaVerify(pubkey, hashData, sigBytes))
        throw new Error("Signature invalid");
    // constuct the invoice
    const result = new invoice_1.Invoice();
...
function parsePrefix(prefix) {
    ...
    // the actual pico value of the
        ...
        value = BigInt(tempValue) * hrp_pico_1.hrpToPico(multiplier);
